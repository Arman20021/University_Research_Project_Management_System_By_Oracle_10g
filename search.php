...
<div class="card p-5">
  ...
  <div class="space-y-3">
    <?php foreach ($results as $r): ?><a class="block rounded-2xl border border-slate-200 p-4 transition hover:bg-sky-50" href="<?= e($r['url']) ?>">
        <div class="flex items-center justify-between">
          <p class="font-semibold text-slate-900"><?= e($r['label']) ?></p><span class="rounded-full bg-slate-100 px-2.5 py-1 text-xs text-slate-600"><?= e($r['type']) ?></span>
        </div>
        <p class="mt-1 text-sm text-slate-500"><?= e($r['meta']) ?></p>
      </a><?php endforeach; ?>
