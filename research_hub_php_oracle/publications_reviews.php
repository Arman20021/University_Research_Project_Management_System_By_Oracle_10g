...
$q = strtolower(getq('q'));
$params=[];
$sql = "SELECT pub.publication_id, pub.project_id, p.title, TO_CHAR(pub.publication_date,'YYYY-MM-DD') publication_date, pub.doi_number, pub.publisher, pub.reviewer_id, r.reviewer_name, pub.review_status, pub.comments FROM publications pub LEFT JOIN projects p ON p.project_id=pub.project_id LEFT JOIN reviewers r ON r.reviewer_id=pub.reviewer_id";
if ($q !== '') { $sql .= " WHERE LOWER(pub.publication_id || ' ' || pub.doi_number || ' ' || pub.publisher || ' ' || pub.review_status || ' ' || r.reviewer_name || ' ' || p.title) LIKE :q"; $params['q'] = '%' . $q . '%'; }
...
$projects = project_options(); $reviewers = reviewer_options();
...
<div class="card mb-6 p-5">
  ...
  <form method="post" class="mt-5 grid gap-4 md:grid-cols-2">
    ...
    <?php input_field('Publication Date','publication_date',$edit['publication_date'] ?? date('Y-m-d'),'date'); input_field('DOI Number','doi_number',$edit['doi_number'] ?? ''); input_field('Publisher','publisher',$edit['publisher'] ?? ''); ?>
...
<div class="grid gap-6 xl:grid-cols-3">
  <div class="card p-5 xl:col-span-2">
    ...
    <div class="table-scroll rounded-2xl border border-slate-200"><table class="min-w-full divide-y divide-slate-200 text-sm">
      <thead class="bg-slate-50 text-left text-slate-600"><tr><th class="px-4 py-3">Publication ID</th><th class="px-4 py-3">Publication Date</th><th class="px-4 py-3">DOI Number</th><th class="px-4 py-3">Publisher</th><th class="px-4 py-3">Reviewer</th><th class="px-4 py-3">Review Status</th><?php if($isAdmin): ?><th class="px-4 py-3 text-right">Actions</th><?php endif; ?></tr></thead>
      <tbody class="divide-y divide-slate-100 bg-white">
      <?php foreach($rows as $r): ?>
        <tr><td class="px-4 py-3"><?= e($r['publication_id']) ?></td><td class="px-4 py-3"><?= e($r['publication_date']) ?></td><td class="px-4 py-3"><?= e($r['doi_number']) ?></td><td class="px-4 py-3"><?= e($r['publisher']) ?></td><td class="px-4 py-3"><?= e($r['reviewer_name']) ?></td><td class="px-4 py-3"><?= status_badge($r['review_status']) ?></td><?php if($isAdmin): ?><td class="px-4 py-3"><div class="flex justify-end gap-2"><a class="btn btn-outline px-3 py-2" href="publications_reviews.php?edit=<?= urlencode($r['publication_id']) ?>">✎</a><form method="post" onsubmit="return confirmDelete('Delete this publication?')"><input type="hidden" name="action" value="delete"><input type="hidden" name="publication_id" value="<?= e($r['publication_id']) ?>"><button class="btn btn-danger px-3 py-2">🗑</button></form></div></td><?php endif; ?></tr>
      <?php endforeach; ?>
      </tbody>
    </table></div>
  </div>
  <div class="space-y-4">
    <?php foreach(array_slice($rows,0,5) as $r): ?>
      <div class="card p-5"><h3 class="text-base font-semibold"><?= e($r['publication_id']) ?></h3><p class="mt-1 text-sm text-slate-500"><?= e($r['publisher']) ?></p><div class="mt-4 space-y-3 text-sm"><div class="flex items-center justify-between"><span class="text-slate-500">Reviewer</span><span class="font-medium"><?= e($r['reviewer_name']) ?></span></div><div class="flex items-center justify-between"><span class="text-slate-500">Status</span><?= status_badge($r['review_status']) ?></div><div class="rounded-xl bg-slate-50 p-3 text-slate-600"><?= e($r['comments']) ?></div></div></div>
    <?php endforeach; ?>
  </div>
</div>
