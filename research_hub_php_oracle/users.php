...
if ($entity === 'students') {
    ...
    if ($q !== '') { $sql .= " WHERE LOWER(s.student_id || ' ' || s.student_name || ' ' || s.phone || ' ' || s.email || ' ' || d.department_name) LIKE :q"; $params['q'] = '%' . $q . '%'; }
...
if ($entity === 'supervisors') {
    ...
    if ($q !== '') { $sql .= " WHERE LOWER(s.supervisor_id || ' ' || s.supervisor_name || ' ' || s.email || ' ' || s.designation || ' ' || d.department_name) LIKE :q"; $params['q'] = '%' . $q . '%'; }
    $sql .= ' ORDER BY s.supervisor_name';
}
if ($entity === 'reviewers') {
    $sql = "SELECT * FROM reviewers";
    if ($q !== '') { $sql .= " WHERE LOWER(reviewer_id || ' ' || reviewer_name || ' ' || designation || ' ' || email || ' ' || phone) LIKE :q"; $params['q'] = '%' . $q . '%'; }
...
<div class="card mb-6 p-5">
  ...
  <form method="post" class="mt-5 grid gap-4 md:grid-cols-2">
    ...
    <?php if ($entity === 'students'): ?>
      ...
      <?php input_field('Student ID','student_id',$edit['student_id'] ?? ''); input_field('Name','student_name',$edit['student_name'] ?? ''); input_field('Phone Number','phone',$edit['phone'] ?? ''); input_field('Date of Birth','dob',$edit['dob'] ?? '', 'date'); input_field('Email Address','email',$edit['email'] ?? '', 'email'); ?>
    ...
    <?php elseif ($entity === 'supervisors'): ?>
      ...
      <?php input_field('Supervisor ID','supervisor_id',$edit['supervisor_id'] ?? ''); input_field('Name','supervisor_name',$edit['supervisor_name'] ?? ''); input_field('Email Address','email',$edit['email'] ?? '', 'email'); input_field('Room No','room_no',$edit['room_no'] ?? ''); input_field('Designation','designation',$edit['designation'] ?? ''); ?>
      <label><span class="form-label">Department</span><select class="input" name="department_id"><?php foreach($departments as $d): ?><option value="<?= e($d['department_id']) ?>" <?= selected($edit['department_id'] ?? '', $d['department_id']) ?>><?= e($d['department_name']) ?></option><?php endforeach; ?></select></label>
    <?php else: ?>
      <input type="hidden" name="old_id" value="<?= e($edit['reviewer_id'] ?? '') ?>">
      <?php input_field('Reviewer ID','reviewer_id',$edit['reviewer_id'] ?? ''); input_field('Name','reviewer_name',$edit['reviewer_name'] ?? ''); input_field('Designation','designation',$edit['designation'] ?? ''); input_field('Email Address','email',$edit['email'] ?? '', 'email'); input_field('Phone Number','phone',$edit['phone'] ?? ''); ?>
    <?php endif; ?>
...
<div class="card p-5">
  ...
  <div class="table-scroll rounded-2xl border border-slate-200"><table class="min-w-full divide-y divide-slate-200 text-sm">
    <thead class="bg-slate-50 text-left text-slate-600"><tr>
      <?php if($entity==='students'): ?><th class="px-4 py-3">Student ID</th><th class="px-4 py-3">Name</th><th class="px-4 py-3">Phone</th><th class="px-4 py-3">DOB</th><th class="px-4 py-3">Email</th><th class="px-4 py-3">Department</th><?php endif; ?>
      <?php if($entity==='supervisors'): ?><th class="px-4 py-3">Supervisor ID</th><th class="px-4 py-3">Name</th><th class="px-4 py-3">Email</th><th class="px-4 py-3">Room</th><th class="px-4 py-3">Designation</th><th class="px-4 py-3">Department</th><?php endif; ?>
      <?php if($entity==='reviewers'): ?><th class="px-4 py-3">Reviewer ID</th><th class="px-4 py-3">Name</th><th class="px-4 py-3">Designation</th><th class="px-4 py-3">Email</th><th class="px-4 py-3">Phone</th><?php endif; ?>
    ...
    <tbody class="divide-y divide-slate-100 bg-white">
      <?php foreach($rows as $r): $id = $r[$entity==='students'?'student_id':($entity==='supervisors'?'supervisor_id':'reviewer_id')]; ?>
        <tr>
          ...
          <td class="px-4 py-3"><div class="flex justify-end gap-2"><a class="btn btn-outline px-3 py-2" href="users.php?entity=<?= e($entity) ?>&edit=<?= urlencode($id) ?>">✎</a><form method="post" onsubmit="return confirmDelete('Delete this record?')"><input type="hidden" name="entity" value="<?= e($entity) ?>"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?= e($id) ?>"><button class="btn btn-danger px-3 py-2">🗑</button></form></div></td>
        </tr>
      <?php endforeach; ?>
    </tbody>
  </table></div>
