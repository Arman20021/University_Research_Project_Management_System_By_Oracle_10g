  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
      ...
      db_execute("INSERT INTO projects (project_id,title,publication_status,submission_date,supervisor_id,funding,publication_info,reviewer_id,status,progress,notes,created_by) VALUES (:id,:title,:publication_status,TO_DATE(:submission_date,'YYYY-MM-DD'),:supervisor_id,:funding,:publication_info,:reviewer_id,'Pending',20,:notes,:created_by)", [
        ...
        'publication_info' => 'Not linked yet',
      ...
      if ($currentUser && strtolower($currentUser['role']) === 'user') {
        $currentStudent = db_fetch_one(
          "SELECT student_id 
          FROM students 
        ...
        if ($currentStudent) {
          $studentIds[] = $currentStudent['student_id'];
        }
      }
      ...
      replace_project_students($projectId, $studentIds);
      $publicationId = 'PUB' . date('His');
      ...
      db_execute(
        "INSERT INTO publications (
          ...
          :p_reviewer_id,
          'Pending',
          'Project submitted and waiting for review.'
      )",
        [
    ...
    <div class="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
      <form method="post" class="space-y-4">
        <div class="grid gap-4 md:grid-cols-2">
          <?php input_field('Project ID', 'project_id', 'PRJ' . date('His'));
          input_field('Project Title', 'project_title');
          select_field('Publication Status', 'publication_status', 'Pending', ['Pending', 'Reviewed', 'Published']);
          input_field('Submission Date', 'submission_date', date('Y-m-d'), 'date'); ?>
          ...
          <div class="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 text-center">
            ...
            <p class="mt-2 font-medium text-slate-700">Upload Documents</p>
            <p class="text-sm text-slate-500">Add a file upload column later if you need physical document storage.</p>
          </div>
        </div>
        <?php $currentUser = current_user(); ?>

        <?php if ($currentUser && strtolower($currentUser['role']) === 'admin'): ?>
          <label>
            ...
            <div class="grid gap-2 rounded-xl border border-slate-200 p-3 md:grid-cols-3">
              <?php foreach ($students as $s): ?>
                <label class="flex items-center gap-2 text-sm">
                  <input type="checkbox" name="students[]" value="<?= e($s['student_id']) ?>">
                  <?= e($s['student_name']) ?>
                </label>
              <?php endforeach; ?>
            </div>
        ...
        <?php else: ?>
          <div class="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
            This project will be submitted under your own student account.
          </div>
        <?php endif; ?>
      ...
      <div class="card bg-slate-50 p-5 shadow-none">
        <h3 class="text-base font-semibold">Submission Guide</h3>
        <p class="mt-1 text-sm text-slate-500">Recommended steps for a complete and review-ready submission</p>
        <div class="mt-4 space-y-3 text-sm text-slate-600">
          <div class="rounded-xl bg-white p-3">Add a precise title and check your publication status.</div>
          <div class="rounded-xl bg-white p-3">Select the right supervisor and associated students.</div>
          <div class="rounded-xl bg-white p-3">Upload supporting files before final submission.</div>
          <div class="rounded-xl bg-white p-3">Use save draft if you need time before sending it for review.</div>
        </div>
      </div>
    </div>
  </div>
