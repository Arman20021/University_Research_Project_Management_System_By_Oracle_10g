  ...
  <div class="table-scroll rounded-2xl border border-slate-200">
    <table class="min-w-full divide-y divide-slate-200 text-sm">
      <thead class="bg-slate-50 text-left text-slate-600">
        <tr>
          <th class="px-4 py-3">Department ID</th>
          <th class="px-4 py-3">Department Name</th>
          <th class="px-4 py-3">Office Room</th>
      ...
      <tbody class="divide-y divide-slate-100 bg-white">
        <?php foreach ($departments as $item): ?>
          <tr>
            <td class="px-4 py-3"><?= e($item['department_id']) ?></td>
            <td class="px-4 py-3 font-medium"><?= e($item['department_name']) ?></td>
            <td class="px-4 py-3"><?= e($item['office_room']) ?></td>
            <td class="px-4 py-3"><?= e($item['email']) ?></td>
            <td class="px-4 py-3"><?= e($item['university']) ?></td>
            <td class="px-4 py-3">
              <div class="flex justify-end gap-2"><a class="btn btn-outline px-3 py-2" href="departments.php?edit=<?= urlencode($item['department_id']) ?>">✎</a>
                <form method="post" onsubmit="return confirmDelete('Delete department <?= e($item['department_name']) ?>?')"><input type="hidden" name="action" value="delete"><input type="hidden" name="department_id" value="<?= e($item['department_id']) ?>"><button class="btn btn-danger px-3 py-2">🗑</button></form>
              </div>
            </td>
          </tr>
        <?php endforeach; ?>
