  ...
  <?php foreach ($stats as $item): ?>
    <div class="card p-5">
      <div class="flex items-center justify-between">
        <div>
          <p class="text-sm text-slate-500"><?= e($item[0]) ?></p>
          <p class="mt-2 text-2xl font-semibold text-slate-900"><?= e($item[1]) ?></p>
        </div>
  ...
  <div class="space-y-6 xl:col-span-2">
        ...
        <div class="mt-4 space-y-4">
          <?php foreach (array_slice($recentProjects, 0, 4) as $row): ?>
            <div class="flex items-center justify-between gap-3 rounded-xl border border-slate-100 p-3">
              <div class="min-w-0">
                <p class="truncate font-medium text-slate-800"><?= e($row['title']) ?></p>
                <p class="text-sm text-slate-500"><?= e($row['supervisor_name'] ?: 'No supervisor') ?></p>
              </div>
      ...
      <div class="card p-5">
        ...
        <div class="mt-4 space-y-4">
          <?php foreach (array_slice($recentPubs, 0, 4) as $row): ?>
            <div class="flex items-center justify-between gap-3 rounded-xl border border-slate-100 p-3">
              <div class="min-w-0">
                <p class="truncate font-medium text-slate-800"><?= e($row['publication_id']) ?></p>
                <p class="text-sm text-slate-500"><?= e($row['publisher']) ?> · <?= e($row['publication_date']) ?></p>
              </div>
    ...
    <div class="card p-5">
      ...
      <div class="mt-5 space-y-4">
        <?php $max = max(1, ...array_map(function ($r) {
          return (int)$r['total'];
        }, $statusRows ?: [['total' => 1]])); ?>
        <?php foreach ($statusRows as $row): $width = ((int)$row['total'] / $max) * 100; ?>
          <div class="space-y-2">
            ...
            <div class="h-3 overflow-hidden rounded-full bg-slate-100">
              <div class="h-full rounded-full bg-sky-500" style="width: <?= e($width) ?>%"></div>
            </div>
          </div>
        <?php endforeach; ?>
      </div>
